...
test('parses a test id', () => {
    const parsed = identifiers.parse(REQ,  'test:reboot');
...
test('parses a vulnerabilities id', () => {
    const parsed = identifiers.parse(REQ, 'vulnerabilities:CVE-2017-5715');
...
test('parses patch (erratum) id', () => {
    const parsed = identifiers.parse(REQ, 'patch-advisory:RHBA-2021:0439');
...
test('parses patch (package) id', () => {
    const parsed = identifiers.parse(REQ, 'patch-package:libstdc++-8.3.1-5.1.el8.x86_64');
...
test('parses a ssg id', () => {
    const parsed = identifiers.parse(REQ, 'ssg:rhel7|pci-dss|xccdf_org.ssgproject.content_rule_sshd_disable_root_login');
