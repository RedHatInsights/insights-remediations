...
[request, requestLegacy].forEach((req, index) => {
    ...
    describe(`diagnosis${index === 1 ? ' (legacy path)' : ''}`, function () {
        ...
        test('returns all details information for the given system with branch_id', async() => {
            const {body} = await req
            .get('/v1/diagnosis/9a212816-a472-11e8-98d0-529269fb1459?branch_id=09899741-6294-411d-a5c6-3d0eac23c43d')
            .set(auth.cert01)
            .expect(200);
        ...
        test('400s on invalid batch id', async () => {
            const {id, header} = reqId();
            const {body} = await req
            .get('/v1/diagnosis/9a212816-a472-11e8-98d0-529269fb1459?branch_id=foo')
            .set(header)
            .set(auth.cert01)
            .expect(400);
