...
exports.getResolutions = errors.async(async function (req, res) {
    const result = await getResolutions(req, req.params.issue);
...
exports.resolutionsBatch = errors.async(async function (req, res) {
    ...
    let result = _(issues).keyBy().mapValues(issue => getResolutions(req, issue)).value();
...
async function getResolutions (req, identifier)  {
    const id = identifiers.parse(req, identifier);
