...
exports.createPlaybookRun = async function (req, recipients, exclude, remediation, username) {
    ...
    // dispatch requests
    const response = await dispatchWorkRequests(workRequests, playbook_run_id, req);
...
//----------------------------------------------
async function dispatchWorkRequests (workRequests, playbook_run_id, req) {
    try {
        probes.workRequest(workRequests, playbook_run_id);
        const response = await dispatcher.postV2PlaybookRunRequests(workRequests, req);
