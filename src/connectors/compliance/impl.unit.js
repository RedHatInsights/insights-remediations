...
describe('compliance impl (v2)', function () {
    ...
    test('obtains rule info', async function () {
        ...
        const result = await impl.getRule('xccdf_org.ssgproject.content_rule_sshd_disable_root_login', '0021d5a8-6573-4766-8bfd-5f5eab59015c');
        result.should.property('ref_id', 'xccdf_org.ssgproject.content_rule_sshd_disable_root_login');
        result.should.property('title', 'Disable SSH Root Login');
        ...
        await impl.getRule('xccdf_org.ssgproject.content_rule_sshd_disable_root_login', '0021d5a8-6573-4766-8bfd-5f5eab59015c');
        cache.get.callCount.should.equal(2);
        cache.setex.callCount.should.equal(1);
    });
    ...
    test('returns empty array on unknown resolution for rule', async function () {
        ...
        await expect(impl.getRule('unknown-rule', '0021d5a8-6573-4766-8bfd-5f5eab59015c')).resolves.toBeNull();
    ...
    test('status code handling', async function () {
        base.mockRequestStatusCode();
        await expect(impl.getRule('unknown-rule', '0021d5a8-6573-4766-8bfd-5f5eab59015c')).rejects.toThrow(errors.DependencyError);
    });

    test('403 response handling', async function () {
        base.mockRequestStatusCode(403);
        await expect(impl.getRule('xccdf_org.ssgproject.content_rule_sshd_disable_root_login', '0021d5a8-6573-4766-8bfd-5f5eab59015c')).resolves.toBeNull();
    });
});
