...
// Fix remediation plan names that have leading/trailing non-breaking whitespace
...
exports.up = async (q) => {
    ...
    // get list of remediation plans with leading / trailing non-breaking whitespace
    const target_sql = "SELECT name, tenant_org_id, id FROM remediations WHERE name ~ '^\u00A0.*' OR name ~ '.*\u00A0$';";
    const [target_plans] = await q.sequelize.query(target_sql);

    for (const plan of target_plans) {
        // remove leading/trailing non-breaking whitespace from target plan name
